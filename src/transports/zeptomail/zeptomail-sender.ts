...
/**
 * ZeptoMail (Zoho Mail transactional email API) transport adapter.
 *
 ...
 */
export class ZeptoMailSender implements IEmailSender {
  ...
  constructor(config: ZeptoMailConfig) {
    if (!config.apiToken || typeof config.apiToken !== 'string') {
      throw new ConfigurationError('ZeptoMailSender: Missing or invalid apiToken.');
    }
    if (!config.from) {
      throw new ConfigurationError('ZeptoMailSender: Missing required default from address.');
    }
    ...
    if (!this.defaultFrom.address) {
      throw new ConfigurationError('ZeptoMailSender: Configured from address must not be empty.');
    }

    this.fetchImpl = config.fetch ?? globalThis.fetch;
    if (typeof this.fetchImpl !== 'function') {
      throw new ConfigurationError(
        'ZeptoMailSender: No global fetch implementation found. Pass a custom fetch to ZeptoMailConfig if running on Node < 18.'
      );
    }
  }
  ...
  async send(options: SendMailOptions): Promise<SendResult> {
    ...
    if (toRecipients.length === 0) {
      throw new TransportError('ZeptoMailSender: At least one recipient (to) must be specified.', {
    ...
    try {
      ...
      throw new TransportError(`ZeptoMailSender: Network request failed: ${err instanceof Error ? err.message : String(err)}`, {
        transportName: this.name,
        cause: err,
      });
    }
    ...
    if (!response.ok) {
      ...
      throw new TransportError(`ZeptoMailSender: Failed to send email - ${errorMessage}`, {
        transportName: this.name,
